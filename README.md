...
```bash
python scripts/western_pan.py <年> <月> <日> <时> <分> <城市>

# 示例
python scripts/western_pan.py 1990 5 15 14 30 北京
```

### 七政四余排盘

```bash
python scripts/qizheng_pan.py <年> <月> <日> <时> <分> [城市]

# 示例
python scripts/qizheng_pan.py 1990 5 15 14 30 北京
```
...
| 西方占星 | 基于 ephem 地心黄道坐标 | 宫位使用简化 Placidus 近似，专业用途请用专业软件 |
| 七政四余 | 基于 `pyswisseph` 精密星历 | 四余星接入真实月交点与远近地点；二十八宿采用传统宿度并岁差修正；建议用专业软件复核 |
